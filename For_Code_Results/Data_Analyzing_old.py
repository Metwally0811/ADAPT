...
def highload(row):
   ...
   return 1 if (row['PDSCH Phy Throughput (kbps)'] < TARGET_THROUGHPUT) and (row['Bad Coverage'] == 0) and (row['Overlapping'] == 0) and (row['Overshooting'] == 0) and (row['Intra-Frequency Handover'] == 0) and (row['Inter-Frequency Handover'] == 0) and (row['Number of PDSCH Resource Blocks'] < MIN_PRB) else 0
    
    # rsrp_condition=row['Serving Cell RSRP (dBm)'] > TARGET_RSRP
    #SINR_RSRQ_Condition= (row['Serving Cell RSRQ (dB)'] < MIN_RSRQ) or (row['Serving Cell RS SINR (dB)'] < MIN_SINR)
    # RB_condition= row['Number of PDSCH Resource Blocks'] < MIN_PRB

    #return  1 if row['PDSCH Phy Throughput (kbps)'] < TARGET_THROUGHPUT and rsrp_condition and SINR_RSRQ_Condition and RB_condition else 0
...
#data = data.drop(columns=['rsrp_rank', 'sinr_rank', 'prbs_rank', 'dist_rank'])
